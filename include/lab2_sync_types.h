...
pthread_mutex_t fg_mutex_queue;
pthread_mutex_t fg_mutex_hash[HASH_SIZE];

/* Structure for thread, hash, queue */
...
typedef struct q_node {
	struct q_node* next;
	struct q_node* prev;
	struct q_node* hnext; // link for hash node
	struct q_node* hprev; // link for hash node
	int data;
} queue_node;
